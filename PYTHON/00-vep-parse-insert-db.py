...
for file in glob.glob("*.txt"):
    ...
    with open(file) as f:
        ...
        next(reader, None)
        for row in reader:
            new_row = [file,Patient]+row
            if row[4] == 'HIGH':
                if len(row) == 51:
                    cursor.execute(sql1.format(*new_row))
                else:
                    string = ' '.join(row)
                    new_row = [file,Patient,string]
